...
/* On PS3 RARCH_PATH_CORE may temporarily contain the content path while
 * switching games.  Never persist that value: Salamander can only launch a
 * SELF from the cores directory. */
static bool config_salamander_core_path_is_valid(const char *path)
{
...
void config_load_file_salamander(void)
{
   ...
   if (config_get_path(config, "libretro_path",
       ...
       config_salamander_core_path_is_valid(libretro_path))
...
void config_save_file_salamander(void)
{
   ...
   if (!config_salamander_core_path_is_valid(libretro_path))
   {
      RARCH_WARN("[Config]: Refusing to save non-core Salamander path: %s.\n",
            string_is_empty(libretro_path) ? "(empty)" : libretro_path);
      return;
   }
